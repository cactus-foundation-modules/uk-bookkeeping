...
export async function assertDateNotInClosedPeriod(date: Date): Promise<void> {
  ...
  `
  const row = rows[0]
  if (row) throw new BackdatedIntoClosedPeriodError(row.id)
}

/** The period a date falls in, if any is closed. Used to offer the correction route. */
export async function findClosedPeriodForDate(date: Date): Promise<string | null> {
  const rows = await prisma.$queryRaw<{ id: string }[]>`
    SELECT "id" FROM "bk_vat_periods"
    WHERE "status" IN ('finalised', 'submitted')
      AND ${date}::date BETWEEN "start_date" AND "end_date"
